...
const NON_IO = {
  ...
  RST: '重置腳。高態持續兩個機器週期就重置。', X1: '石英振盪接腳 XTAL1。', X2: '石英振盪接腳 XTAL2。',
  EA: '外部程式記憶體致能。接 VCC 表示用晶片內部的程式記憶體。', ALE: '位址鎖存致能，接外部記憶體才用得到。',
  PSEN: '外部程式記憶體讀取訊號。', WP: 'EEPROM 防寫腳。', VSS: '接地。', VDD: '電源。', VO: 'LCM 對比調整。',
  ...
  A: 'LCM 背光正極。', K: 'LCM 背光負極。', G: 'RGB LED 綠。', B: 'RGB LED 藍。', '中': 'LCM 型式選擇中間腳。',
...
export class BoardView {
  ...
  _defs() {
    ...
    const g1 = el('linearGradient', { id: 'pcb', x1: 0, y1: 0, x2: 0, y2: 1 }, defs);
    el('stop', { offset: '0%', 'stop-color': '#b02430' }, g1);
    el('stop', { offset: '100%', 'stop-color': '#8a1a22' }, g1);
  ...
  _screw(g, x, y) {
    el('circle', { cx: P(x), cy: P(y), r: P(8.5), fill: '#cfcfcf', stroke: '#8a8a8a', 'stroke-width': 1.5 }, g);
    el('circle', { cx: P(x), cy: P(y), r: P(4), fill: '#7c7c7c' }, g);
  }
  _chip(g, x, y, w, h, label, help, vertical) {
    const c = el('g', {}, g);
    el('rect', { x: P(x), y: P(y), width: P(w), height: P(h), rx: 2, fill: '#26262a', stroke: '#111' }, c);
    const n = Math.max(2, Math.round((vertical ? h : w) / 9));
    for (let i = 0; i < n; i++) {
      const t = n === 1 ? 0 : i / (n - 1);
      if (vertical) {
        el('rect', { x: P(x) - 4, y: P(y + 3 + t * (h - 6)), width: 4, height: 3, fill: '#b9b9b9' }, c);
        el('rect', { x: P(x + w), y: P(y + 3 + t * (h - 6)), width: 4, height: 3, fill: '#b9b9b9' }, c);
      } else {
        el('rect', { x: P(x + 3 + t * (w - 6)), y: P(y) - 4, width: 3, height: 4, fill: '#b9b9b9' }, c);
        el('rect', { x: P(x + 3 + t * (w - 6)), y: P(y + h), width: 3, height: 4, fill: '#b9b9b9' }, c);
      }
    }
    if (label) txt(c, P(x + w / 2), P(y + h / 2 + 1.8), label, { 'font-size': 12, fill: '#b5b5b5', 'text-anchor': 'middle' });
  ...
  _header(spec) {
    ...
    el('rect', { x: P(spec.x - 3.5), y: P(spec.y - 3.5), width: P(w), height: P(h), rx: 2, fill: '#101015', stroke: '#000' }, g);
    ...
    spec.pins.forEach((pin, i) => {
      ...
      const base = clickable ? GOLD : '#9a9a9a';
      const grp = el('g', clickable ? { class: 'clickable' } : {}, g);
      el('rect', { x: px - P(4), y: py - P(4), width: P(8), height: P(8), fill: 'transparent' }, grp);
      const dot = el('rect', { x: px - 3.4, y: py - 3.4, width: 6.8, height: 6.8, rx: 1.2, fill: base, stroke: on ? '#6d4a12' : '#5a5a5a' }, grp);
      dot._base = base;
      ...
      if (dev) {
        ...
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`, (spec.note || '') + '<br>點一下開始拉線，再點另一支腳接上。');
        ...
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`,
          `這支腳在電氣上就是 <b>${port}</b>。<br>` +
          (MAIN_FIXED[port] ? `板上已經焊了：<b>${MAIN_FIXED[port]}</b>（焊死不影響，這支腳照樣可以再拉線）<br>` : '這支腳沒焊任何東西。<br>') +
          '點一下開始拉線，再點另一支腳接上。');
        ...
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`,
          rail === 'gnd' ? '接地腳。<br>點一下抓住它，再點任何一支埠腳，就等於用杜邦線把那支腳<b>拉到 0</b>。<br>課本 2-7-2 那句「用杜邦線碰 GND」就是這個動作。'
                         : '+5V 電源腳。<br>點一下抓住它，再點任何一支埠腳，就等於用杜邦線把那支腳<b>拉到 1</b>。<br>⚠ 程式若對被拉高的腳寫 0，實機等於短路，模擬器會標成電氣衝突。');
        (this.railEls[rail] = this.railEls[rail] || []).push({ dot });
      } else if (ctrl) {
        grp.addEventListener('click', (e) => { e.stopPropagation(); this._ctrlClicked(ctrl); });
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`,
          ctrl === 'RST' ? '重置腳。<br>接到 <b>VCC</b> → 晶片被一直按住重置，程式不會跑（模擬器會照做）。<br>接到 GND = 正常執行。'
                         : 'EA（程式記憶體來源）。<br>接到 <b>GND</b> → 89S51 改從外部 ROM 取碼，而這塊板子沒有外接 ROM，實機會抓到空指令。<br>模擬器會警告，但仍照內部 4K ROM 執行。');
        (this.ctrlEls[ctrl] = this.ctrlEls[ctrl] || []).push({ dot });
      } else if (jmp) {
        grp.addEventListener('click', (e) => { e.stopPropagation(); this._jumperClicked(jmp); });
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`,
          '跳線帽：短接這兩支腳，就把 <b>LM35 的輸出送進 MCP3202 的 CH1</b>。<br>拔掉的話 CH1 變成浮接，程式讀到 0V。<br>點一下切換。');
        (this.jmpEls[jmp] = this.jmpEls[jmp] || []).push({ x: px, y: py, dot });
      } else if (kind === 'out') {
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`,
          val === 'adcCh1' ? 'MCP3202 的 CH1 類比輸入。<br>電壓來源是左邊那個 JP11 跳線帽（接上 = LM35）。<br>目前值在下方「類比 / 環境」面板。'
            : `MCP4822 的類比輸出 ${val === 'dacA' ? 'CHA' : 'CHB'}。<br>實機是接示波器或喇叭；<b>目前電壓在下方「類比 / 環境」面板</b>的長條上。<br>類比訊號沒有可以接的數位腳位。`);
      } else if (kind === 'fixed') {
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`,
          '步進馬達本體的線，<b>出廠就插在這裡</b>，不用也不能自己接。<br>要接的是 JP7 那四支控制腳（B2 B1 A2 A1）。');
      } else {
        this._tip(grp, `${spec.id} 第 ${i + 1} 腳 — ${name}`,
          kind === 'na' ? `<b>這支腳模擬不了</b>：${val}` :
          (NON_IO[name] || '這支腳沒有分類到任何角色。'));
      }
    });
    return g;
  }
  ...
  _buildMain() {
    ...
    this._tip(rst, 'RESET 鈕', '重置晶片，程式從位址 0 重新執行。');

    const bz = el('g', {}, g);
    el('circle', { cx: P(72), cy: P(300), r: P(25), fill: '#1b2038', stroke: '#0c0f1c', 'stroke-width': 2 }, bz);
    el('circle', { cx: P(72), cy: P(300), r: P(18), fill: '#252c4c' }, bz);
    ...
    this._tip(bz, '蜂鳴器 B1 — P3.7', '低態動作：寫 <b>0</b> 導通發聲。<br>要出聲就讓 P3.7 以想要的頻率翻轉。');

    const zif = el('g', {}, g);
    el('rect', { x: P(146), y: P(110), width: P(86), height: P(252), rx: 4, fill: '#1f7a34', stroke: '#12551f', 'stroke-width': 2 }, zif);
    el('rect', { x: P(164), y: P(126), width: P(58), height: P(224), rx: 2, fill: '#1a1a1a', stroke: '#2f2f2f' }, zif);
    for (let i = 0; i < 20; i++) {
      el('rect', { x: P(157), y: P(131 + i * 11.2), width: P(8), height: P(5), rx: 1, fill: '#cfcfcf' }, zif);
      el('rect', { x: P(221), y: P(131 + i * 11.2), width: P(8), height: P(5), rx: 1, fill: '#cfcfcf' }, zif);
    }
    txt(zif, P(172), P(121), 'LTC', { 'font-size': 15, fill: '#7ee39a', 'font-style': 'italic' });
    el('circle', { cx: P(206), cy: P(118), r: P(4), fill: '#cfcfcf' }, zif);
    el('circle', { cx: P(189), cy: P(356), r: P(4), fill: '#cfcfcf' }, zif);
    txt(zif, P(193), P(232), 'Atmel', { 'font-size': 15, fill: '#c8c8c8', 'text-anchor': 'middle' });
    txt(zif, P(193), P(246), '89S51', { 'font-size': 18, fill: '#eee', 'text-anchor': 'middle', 'font-weight': 'bold' });
    this._tip(zif, 'AT89S51 主角晶片', '8 位元微控制器，12MHz → 1 個機械週期 = 1 µs。<br>4K 程式記憶體、128 位元組 RAM。<br>綠色是零插拔力(ZIF)座。');
    ...
    for (let i = 0; i < 8; i++) {
      ...
      this._tip(hit, `指撥開關 SW1-${i + 1} — P0.${i}`, '撥到 ON（上）→ 接地，程式讀到 <b>0</b>。<br>OFF → 經 RP1 10K 上拉，讀到 <b>1</b>。<br>⚠ 插著 LCM 時要全部撥 OFF（共用 P0）。');
    ...
    this._tip(tc74, 'U3 TC74A0-5.0VAT', 'I²C 數位溫度感測器，位址 1001000。<br>要用它得先把 JP1 的 SDA / SCL 拉線到埠。');
    ...
    this._tip(vr1, 'VR1 20K 可變電阻', '類比輸入來源，接到 MCP3202 的 <b>CH0</b>。<br>用下方「類比 / 環境」的滑桿調整它的電壓。');
    ...
    this._tip(this.lcmHit, 'LCM 16×2 字元液晶', '插在 JP2 上。實體是蓋在板子中間，這裡畫在左上角才不會擋住下面的元件。<br>接法焊死：RS=P3.2、R/W=P3.1、EN=P3.0、D0–D7=P0。<br>點一下可拔除。');

    // 沒插 LCM 時，在它該在的位置畫一個空插槽，點一下就插上
    this.lcmSlot = el('g', { class: 'clickable' }, g);
    el('rect', { x: P(20), y: P(2), width: P(112), height: P(56), rx: 3, fill: 'rgba(0,0,0,.18)',
      stroke: '#9fb4cc', 'stroke-width': 2, 'stroke-dasharray': '7 5' }, this.lcmSlot);
    txt(this.lcmSlot, P(76), P(34), 'LCM（接 JP2）', { 'font-size': 13, fill: '#c3ccd8', 'text-anchor': 'middle' });
    this.lcmSlot.addEventListener('click', toggleLcm);
    this._tip(this.lcmSlot, 'LCM 16×2 字元液晶（未插上）',
      '點一下把模組插到 JP2 上。<br>接法焊死：RS=P3.2、R/W=P3.1、EN=P3.0、D0–D7=P0。<br>⚠ 插上之後 P0 被佔用，SW1 要全部撥 OFF。');
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const grp = el('g', {}, g);
      el('circle', { cx, cy, r: P(7), fill: '#3a2a08' }, grp);
      const halo = el('circle', { cx, cy, r: P(13), fill: 'url(#ledglow)', opacity: 0 }, grp);
      const led = el('circle', { cx, cy, r: P(5.4), fill: '#6b5410', stroke: '#241a04' }, grp);
      txt(g, cx, P(383), `DS${i + 1}`, { 'font-size': 12, 'text-anchor': 'middle' });
      this._tip(grp, `LED DS${i + 1} — P1.${i}`, '低態動作：程式寫 <b>0</b> 才亮。<br>陽極經 RP2 接 VCC。');
    ...
    const bhelp = ['外部中斷 0。⚠ 與 LCM 的 RS 線共用 P3.2。', '外部中斷 1。',
      '一般輸入腳。課本 2-7-2 叫你「用杜邦線碰 GND」的就是這隻。', '一般輸入腳。'];
    for (let i = 0; i < 4; i++) {
      const x = bx[i], y = 366;
      ...
      this._tip(b, `按鍵 ${BUTTONS[i].name} — ${BUTTONS[i].sub}`, `按住 = 接地讀到 <b>0</b>，放開 = 1。<br>${bhelp[i]}`);
  ...
  _buildKdm() {
    const g = el('g', {}, this.svg);
    el('rect', { x: P(466), y: P(68), width: P(421), height: P(326), rx: 8, fill: 'url(#pcb)' }, g);
    this._screw(g, 478, 79); this._screw(g, 868, 78); this._screw(g, 479, 382); this._screw(g, 866, 380);
    ...
    // 七段兩組共八位
    this.segs = [];
    for (const [x0, w] of [[485, 187], [674, 188]]) el('rect', { x: P(x0), y: P(63), width: P(w), height: P(70), rx: 3, fill: '#111', stroke: '#2a2a2a' }, g);
    ...
    for (let d = 0; d < 8; d++) {
      ...
      for (const e of s) e.setAttribute('fill', '#3a1208');
      this.segs.push(s);
    }
    const segHit = el('rect', { x: P(485), y: P(63), width: P(377), height: P(70), fill: 'transparent' }, g);
    this._tip(segHit, '八位數七段顯示器（共陽極）', '掃描線拉低選中該位，段線 = 0 該段亮。<br>八位不能同時亮 → 必須 <b>快速輪流掃描</b>，靠視覺暫留看起來才連續。<br>掃太慢就會閃爍，模擬器會如實呈現。');

    // 電晶體、排阻
    for (let i = 0; i < 8; i++)
      el('ellipse', { cx: P(570 + i * 25.4), cy: P(160), rx: P(9), ry: P(11), fill: '#8f8f8f', stroke: '#6a6a6a' }, g);
    txt(g, P(560), P(180), '位選電晶體 ×8', { 'font-size': 12 });

    this._chip(g, 545, 236, 76, 30, 'SN74LS138N', '3 對 8 解碼器：用三支腳解出八條掃描線。');
    this._chip(g, 557, 311, 68, 25, 'ULN2803A', 'U4 ULN2803A 達靈頓陣列：把 89S51 的小電流放大到能推步進馬達。');
    ...
    // 8×8 點矩陣
    this.dots = [];
    el('rect', { x: P(763), y: P(141), width: P(122), height: P(119), rx: 3, fill: '#111', stroke: '#2a2a2a' }, g);
    for (let c = 0; c < 8; c++) for (let r = 0; r < 8; r++)
      this.dots.push(el('circle', { cx: P(771 + c * 15.4), cy: P(149 + r * 15.4), r: P(5.6), fill: '#3a1208' }, g));
    txt(g, P(846), P(138), 'DS3', { 'font-size': 12 });
    const mHit = el('rect', { x: P(763), y: P(141), width: P(122), height: P(119), fill: 'transparent' }, g);
    this._tip(mHit, '8×8 LED 點矩陣 DS3', '行選線與七段的掃描線 <b>共用</b>。<br>一樣要一行一行快速掃描。');
    ...
    for (let row = 0; row < 4; row++) for (let col = 0; col < 4; col++) {
      ...
      el('rect', { x: P(x), y: P(y), width: P(30), height: P(24), rx: 3, fill: '#c9c9c9', stroke: '#8a8a8a' }, k);
      const cap = el('ellipse', { cx: P(x + 15), cy: P(y + 12), rx: P(8), ry: P(6.5), fill: '#5b6070' }, k);
      ...
      this._tip(k, `按鍵 PB${n}（面板印 ${face[row * 4 + col]}）`, `位在 R${row} 列 × C${col} 行。<br>按下 → R${row} 與 C${col} 短路。`);
      this.keys.push(cap);
    }
  ...
  _buildMotor() {
    ...
    el('circle', { cx: mx, cy: my, r: P(62), fill: '#d3d3d3', stroke: '#8f8f8f', 'stroke-width': 2 }, g);
    el('circle', { cx: mx, cy: my, r: P(50), fill: '#bdbdbd' }, g);
    this.rotor = el('g', {}, g);
    for (let i = 0; i < 16; i++) el('rect', { x: mx - P(3), y: my - P(30), width: P(6), height: P(11), rx: 1, fill: '#9a7c2a', transform: `rotate(${i * 22.5} ${mx} ${my})` }, this.rotor);
    el('circle', { cx: mx, cy: my, r: P(17), fill: '#a8892f', stroke: '#6d5a1c', 'stroke-width': 2 }, this.rotor);
    el('circle', { cx: mx, cy: my, r: P(7), fill: '#e8e8e8' }, this.rotor);
    el('rect', { x: mx - P(2), y: my - P(29), width: P(4), height: P(13), rx: 1.5, fill: '#e74c3c' }, this.rotor);
    this._tip(g, '步進馬達（出廠就接好）', '四相單極馬達，全步 1.8°（一圈 200 步）。<br>馬達本體的六條線<b>出廠就插在 KDM+ 的 CN3 上</b>，不用也不能自己接。<br>要接的是 JP7（B2 B1 A2 A1）那四支控制腳 —— 把它們拉到主板的埠腳位。<br>依序給線圈通電就轉，順序反過來就反轉。');
    this.coilDots = [];
    for (let i = 0; i < 4; i++) {
    ...
    this.motorText = txt(this.svg, P(145), P(34), '', { 'font-size': 15 });
  ...
  update() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      led.setAttribute('fill', b > 0.02 ? `rgb(${Math.round(107 + 148 * b)},${Math.round(84 + 140 * b)},${Math.round(16 + 60 * b)})` : '#6b5410');
    ...
    for (let d = 0; d < 8; d++) for (let s = 0; s < 8; s++) {
      ...
      this.segs[d][s].setAttribute('fill', b > 0.02 ? `rgb(${Math.round(58 + 197 * b)},${Math.round(18 + 60 * b)},${Math.round(8 + 28 * b)})` : '#3a1208');
    }
    for (let i = 0; i < 64; i++) {
      const b = gamma(sd.matrix.smooth[i]);
      this.dots[i].setAttribute('fill', b > 0.02 ? `rgb(${Math.round(58 + 197 * b)},${Math.round(18 + 60 * b)},${Math.round(8 + 28 * b)})` : '#3a1208');
    }
    for (let n = 0; n < 16; n++) this.keys[n].setAttribute('fill', sim.keypad.pressed[n] ? '#8fc0e8' : '#5b6070');
