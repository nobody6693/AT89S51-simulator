...
export class BoardView {
  ...
  _defs() {
    ...
    // LED：核心接近白、外圈帶黃，才像真的在發光而不是貼了一塊白斑
    const g2 = el('radialGradient', { id: 'ledglow' }, defs);
    el('stop', { offset: '0%', 'stop-color': '#FFFFF0', 'stop-opacity': 1 }, g2);
    el('stop', { offset: '38%', 'stop-color': '#FFE873', 'stop-opacity': .72 }, g2);
    el('stop', { offset: '100%', 'stop-color': '#FFC933', 'stop-opacity': 0 }, g2);
    // 亮度高時再往外散一圈，讓整塊板子被照到
    const g3 = el('radialGradient', { id: 'ledbloom' }, defs);
    el('stop', { offset: '0%', 'stop-color': '#FFE85C', 'stop-opacity': .42 }, g3);
    el('stop', { offset: '100%', 'stop-color': '#FFD54A', 'stop-opacity': 0 }, g3);
    // 蜂鳴器：琥珀色
    const g4 = el('radialGradient', { id: 'buzzglow' }, defs);
    el('stop', { offset: '0%', 'stop-color': '#FFE9A8', 'stop-opacity': .85 }, g4);
    el('stop', { offset: '45%', 'stop-color': '#FFD166', 'stop-opacity': .45 }, g4);
    el('stop', { offset: '100%', 'stop-color': '#FFB000', 'stop-opacity': 0 }, g4);
  }
  ...
  _chip(g, x, y, w, h, label, vertical) {
    const c = el('g', {}, g);
  ...
  _buildMain() {
    ...
    this.buzzGlow = el('circle', { cx: P(72), cy: P(300), r: P(58), fill: 'url(#buzzglow)', opacity: 0 }, bz);
    this.buzzRing = el('circle', { cx: P(72), cy: P(300), r: P(29), fill: 'none', stroke: '#ffe9a8', 'stroke-width': 4, opacity: 0 }, bz);
    this.buzzRing2 = el('circle', { cx: P(72), cy: P(300), r: P(38), fill: 'none', stroke: '#ffd166', 'stroke-width': 2, opacity: 0 }, bz);
    ...
    // 類比區
    this._chip(g, 340, 262, 46, 34, 'MCP3202');
    this._chip(g, 344, 306, 42, 34, 'MCP4822');
    this._chip(g, 348, 161, 37, 24, '24LC16B');
    const tc74 = el('g', {}, g);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const bloom = el('circle', { cx, cy, r: P(21), fill: 'url(#ledbloom)', opacity: 0 }, grp);
      const halo = el('circle', { cx, cy, r: P(15), fill: 'url(#ledglow)', opacity: 0 }, grp);
      const led = el('circle', { cx, cy, r: P(5.4), fill: '#C6C42A', stroke: '#7E7C18' }, grp);
      txt(g, cx, P(383), `DS${i + 1}`, { 'font-size': 12, 'text-anchor': 'middle' });
      this.leds.push({ led, halo, bloom });
    }
  ...
  _buildKdm() {
    ...
    this._chip(g, 545, 236, 76, 30, 'SN74LS138N');
    this._chip(g, 557, 311, 68, 25, 'ULN2803A');
  ...
  update() {
    ...
    const bzOn = bd > 0.01 && bd < 0.99;
    this.buzzRing.setAttribute('opacity', bzOn ? 1 : 0);
    this.buzzRing2.setAttribute('opacity', bzOn ? 0.6 : 0);
    this.buzzGlow.setAttribute('opacity', bzOn ? 1 : 0);
